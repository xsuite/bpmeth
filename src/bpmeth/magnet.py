...
class Splines_Magnet:
    ...
    def __init__(self, data, h, l_magn, design_field=None, order=3, hgap=0.05, apt=0.05, radius=0.05, nphi=3, plot=False, step=50, in_FS_coord=False, symmetric=False,
                 smin=None, smax=None):
        """
        Magnet consisting of splines, tracked with the (slow) built-in tracker from Hamiltonian. Use RK4_Magnet for speedup and flexibility.
        ...
        if not h==0:
            ...
            if not in_FS_coord:
                ...
                s = np.unique(self.fieldmap.src['s'])
    ...
    def create_fieldexpansion(self, plot=False):
        ...
        for i in range(len(self.segments)): 
            ...
            if self.segments[i][0] < -self.sedge and self.segments[i][1] > self.smin:  # Incoming drift
                expansion = FieldExpansion(b=bb, h=0., nphi=self.nphi)
            ...
            if self.segments[i][1] > -self.sedge and self.segments[i][0] < self.sedge:  # Body part
                expansion = FieldExpansion(b=bb, h=self.h, nphi=self.nphi)
    ...
    def track(self, particles, ivp_opt=None, plot=False):
        sols = []
