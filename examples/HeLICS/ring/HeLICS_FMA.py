...
def calc_tunes(hxm, hym, nturn_tune=200):
    n_part = len(hxm)
    Qx1 = np.zeros(n_part)
    Qx2 = np.zeros(n_part)
    Qy1 = np.zeros(n_part)
    ...
    for i in range(n_part):
        Qx1[i] = nafflib.tune(hxm[i, :nturn_tune], window_order=2, window_type="hann")
        Qx2[i] = nafflib.tune(hxm[i, -nturn_tune:], window_order=2, window_type="hann")
        Qy1[i] = nafflib.tune(hym[i, :nturn_tune,], window_order=2, window_type="hann")
        Qy2[i] = nafflib.tune(hym[i, -nturn_tune:], window_order=2, window_type="hann")
...
def calc_FMA(line, fringe, n_turn=1000, nturn_tune=200, nr=80, nphi=40):
    ...
    Qx1, Qx2, Qy1, Qy2 = calc_tunes(hxm, hym, nturn_tune=nturn_tune)
