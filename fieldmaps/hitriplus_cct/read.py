...
class Magnet:
    ...
    def fit_x(self, rho, rmax, spos, field, radius=0.01, n_r=25, degree=10, plot=False):
        ...
        if plot:
            ...
            fig, ax = plt.subplots()
            ax.scatter(dst.point_data["xFS"], dst.point_data[field], label=f"$B_x(x, y=0, s={spos})$", s=3, color="orange")
            ax.plot(x, yfit, label="Fit", color="red") 
            ax.plot(x, yfitcenter, label="Central fit", color="purple") 
            ax.plot(x, ydiff, label="Derivatives", color="pink") 
